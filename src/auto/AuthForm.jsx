    ...
    const onSubmit = useCallback(async (data) => { // Функция обработки отправки формы
        if (isLogin) { // Если форма для входа
            ...
            if (!isLoggedIn) { // Если вход не удался
                alert("Пользователь не зарегистрирован. Пожалуйста, пройдите регистрацию."); // Уведомляем пользователя 
            ...
            if (!isRegistered) { // Если регистрация не удалась
                alert("Пользователь с таким email уже существует. Пожалуйста, войдите в систему."); // Уведомляем 
                ...
                <Form.Control
                    ...
                    {...register('email', {  // Регистрация поля email с валидацией 
                        ...
                        pattern: {
                            ...
                            message: "Электронная почта должна быть в формате: example@mail.ru или example@gmail.com"  
